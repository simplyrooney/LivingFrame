 ...
 async fetch(request,env){
  ...
  try{
   if(request.method==="GET"&&url.pathname==="/api/health")
    return json({ok:true,service:"living-frame-backend",runtime:"cloudflare-workers",version:"2.1.0"},200,cors);
   ...
   if(request.method==="GET"&&dl){
    ...
    const path=kind==="photo"?f.photo_path:f.video_path;
    if(!path)return json({error:`No ${kind} uploaded for this frame`},404,cors);
    ...
    const date=formatDDMMYYYY(f.created_at);
    const ext=getExtension(path,kind==="photo"?"jpg":"mp4");
...
async function requireUser(request,env){
 const auth=request.headers.get("Authorization")||"",m=auth.match(/^Bearer\s+(.+)$/i);
...
async function requireAdmin(request,env){
 ...
 if(u.data.id!==env.ADMIN_USER_ID)return{ok:false,status:403,error:"Admin access required"};
 return u;
}
