...
export async function handler(event){
 ...
 const path=raw.replace(/^\/api/,"").replace(/^\/\.netlify\/functions\/api/,"")||"/";
 if(path==="/health") return reply(200,{ok:true,service:"living-frame-backend",runtime:"netlify-functions",version:"4.0.0"},origin);

 const {SUPABASE_URL,SUPABASE_ANON_KEY,SUPABASE_SERVICE_ROLE_KEY}=process.env;
 if(!SUPABASE_URL||!SUPABASE_ANON_KEY||!SUPABASE_SERVICE_ROLE_KEY) return reply(500,{error:"Supabase env vars missing"},origin);
 ...
 const ownedFrame=async id=>{
   const {data}=await admin.from("frames").select("*").eq("id",id).eq("user_id",user.id).single();
   return data||null;
 };
 try{
   if(event.httpMethod==="POST" && path==="/frames"){
     ...
     for(let i=0;i<4;i++){
       const {data,error}=await admin.from("frames").insert({
         frame_code:`LF-${makeCode()}`,user_id:user.id,title,status:"draft"
       }).select().single();
   ...
   if(event.httpMethod==="GET" && path==="/frames"){
     const {data,error}=await admin.from("frames").select("*").eq("user_id",user.id).order("created_at",{ascending:false});
   ...
   if(parts[0]==="frames" && parts[1]){
     ...
     if(event.httpMethod==="POST" && parts[2]==="upload-url"){
       ...
       if(!["photo","video"].includes(kind)) return reply(400,{error:"kind must be photo or video"},origin);
       ...
       if(error) return reply(500,{error:error.message},origin);
       return reply(200,{bucket,path:objectPath,token:data.token,signedUrl:data.signedUrl},origin);
     }
     if(event.httpMethod==="POST" && parts[2]==="asset-complete"){
       const frame=await ownedFrame(id);
       if(!frame) return reply(404,{error:"Frame not found"},origin);
       const {kind,path:assetPath}=body;
       if(!["photo","video"].includes(kind)||!assetPath) return reply(400,{error:"kind and path are required"},origin);
       ...
       const {data,error}=await admin.from("frames").update(patch).eq("id",id).eq("user_id",user.id).select().single();
       if(error) return reply(500,{error:error.message},origin);
       return reply(200,data,origin);
     }
   }
   return reply(404,{error:"Route not found",path},origin);
 }catch(err){ return reply(500,{error:err?.message||"Internal server error"},origin); }
}
